...
sing_line_loop <- function(dataset, line, phrase_col) {
  
  # Introduce the day
  
  num_word <- english::ordinal(line)
  line_returned <- glue::glue("On the {num_word} day of Christmas, my true love sent to me")
  ...
  for(i in line:1) {
    if(i == 1) {
      if(line > 1) {
        line_returned <- paste(line_returned, phrases[1], sep = " and ")
      } else {
        line_returned <- paste(line_returned, phrases[1], sep = " ")
      }
    } else {
      line_returned <- paste(line_returned, phrases[i], sep = ", ")
    }
  }
...
sing_line_without_iteration <- function(dataset, line, phrase_col) {
  
  num_word <- english::ordinal(line)
  line_returned <- glue::glue("On the {num_word} day of Christmas, my true love sent to me")
  
  if(line == 1){
    line_returned <- paste(line_returned, phrase_col[1], sep = " ")
    return(line_returned)
  }
  
  gift_lines <- str_c(phrase_col[line:2], collapse = ", ")
  line_returned <- paste(line_returned, gift_lines, sep = " ")
  line_returned <- paste(line_returned, phrase_col[1], sep = " and ")
  return(line_returned)
  
}

# My next idea was to create a helper function that could combine the gifts, which I could then use within lapply. I could not get lapply to work, but this function collapses string lists recursively, which is kinda cool.

recursive_add <- function(char, x = length(char)) {
  if(x == 2) {
    return(char[2])
  }
  char[x] <- str_c(char[x], recursive_add(char, x - 1), sep = ", ")
  return(char[x])
}

sing_line_recursive <- function(dataset, line, phrase_col) {
  ...
  gift_lines <- recursive_add(phrase_col, line)
  line_returned <- paste(line_returned, gift_lines, sep = " ")
  line_returned <- paste(line_returned, phrase_col[1], sep = " and ")
  return(line_returned)
...
map_chr(1:12, ~sing_line(xmas2, .x, xmas$Full.Phrase)) %>%
  str_c(collapse = "\n") %>%
  cat()
